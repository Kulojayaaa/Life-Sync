...
type MealEntry = { breakfast: number; lunch: number; dinner: number; snacks: number };
type MealEntries = Record<string, MealEntry>;
const EMPTY_MEAL_ENTRY: MealEntry = { breakfast: 0, lunch: 0, dinner: 0, snacks: 0 };
...
interface ExpensesProps { initialTab?: string }

export default function Expenses({ initialTab = 'transactions' }: ExpensesProps) {
  const { user } = useAuth();
  ...
  const mealPlanDate = useMemo(() => parseMonthDate(planForm.month), [planForm.month]);
  const mealPlanDays = useMemo(() => {
    const count = endOfMonth(mealPlanDate).getDate();
    return Array.from({ length: count }, (_, index) => {
      const date = new Date(mealPlanDate.getFullYear(), mealPlanDate.getMonth(), index + 1);
      return { day: index + 1, label: format(date, 'EEE, d MMM') };
    });
  }, [mealPlanDate]);
  const foodBudget = useMemo(
    () => budgets
      .filter((budget) => budget.month === mealPlanDate.getMonth() + 1 && budget.year === mealPlanDate.getFullYear() && budget.category_name.toLowerCase().includes('food'))
      .reduce((sum, budget) => sum + Number(budget.amount || 0), 0),
    [budgets, mealPlanDate],
  );
  const mealTotals = mealPlanDays.reduce((totals, date) => {
    const entry = mealEntries[String(date.day)] || EMPTY_MEAL_ENTRY;
    totals.breakfast += Number(entry.breakfast || 0);
    totals.lunch += Number(entry.lunch || 0);
    totals.dinner += Number(entry.dinner || 0);
    totals.snacks += Number(entry.snacks || 0);
    return totals;
  }, { ...EMPTY_MEAL_ENTRY });
  const totalFoodExpenses = Object.values(mealTotals).reduce((sum, amount) => sum + amount, 0);

  useEffect(() => {
    const saved = localStorage.getItem(`life-sync-meal-expenses-${planForm.month}`);
    if (!saved) {
      setMealEntries({});
      return;
    }
  ...
  const saveMealPlan = () => {
    localStorage.setItem(`life-sync-meal-expenses-${planForm.month}`, JSON.stringify(mealEntries));
    toast.success('Daily food expenses saved');
  };

  const updateMealExpense = (day: number, meal: keyof MealEntry, amount: number) => {
    setMealEntries((current) => ({
      ...current,
      [String(day)]: { ...(current[String(day)] || EMPTY_MEAL_ENTRY), [meal]: Math.max(amount || 0, 0) },
    }));
  };
  ...
  const filteredIncome = filteredTransactions.filter((transaction) => transaction.type === 'credit').reduce((sum, transaction) => sum + transaction.amount, 0);
  const filteredExpenses = filteredTransactions.filter((transaction) => transaction.type === 'debit').reduce((sum, transaction) => sum + transaction.amount, 0);
  const hasTransactionFilters = Boolean(txSearch || txTypeFilter !== 'all' || txCategoryFilter !== 'all' || txSpendingFilter !== 'all' || txFromDate || txToDate);
  const clearTransactionFilters = () => {
    setTxSearch(''); setTxTypeFilter('all'); setTxCategoryFilter('all'); setTxSpendingFilter('all'); setTxFromDate(''); setTxToDate('');
  };
  ...
  const pageTitles = {
    transactions: { title: 'Transactions', subtitle: 'Track and manage your income and expenses', icon: ArrowRightLeft },
    accounts: { title: 'Accounts', subtitle: 'Manage balances and financial accounts', icon: Wallet },
    budgets: { title: 'Budgets', subtitle: 'Set and monitor monthly spending limits', icon: Target },
    savings: { title: 'Savings Goals', subtitle: 'Track progress toward your savings targets', icon: PiggyBank },
    emis: { title: 'EMI', subtitle: 'Manage instalments and payment schedules', icon: CreditCard },
    debt: { title: 'Debt Tracker', subtitle: 'Monitor borrowing and repayments', icon: Landmark },
    planner: { title: 'Expense Planner', subtitle: 'Plan cash flow and record daily food expenses', icon: CalendarRange },
    analytics: { title: 'Insights', subtitle: 'Understand your financial trends', icon: BarChart2 },
    reports: { title: 'Reports', subtitle: 'Review account and transaction reports', icon: BarChart2 },
  };
  const pageTitle = pageTitles[initialTab as keyof typeof pageTitles] || pageTitles.transactions;
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><PageIcon className="h-5 w-5" /></span>
              <div>
          ...
          <div className="flex flex-wrap gap-2">
            {initialTab === 'transactions' && (
              <>
                <Button variant="outline" onClick={handleExport} disabled={transactions.length === 0}>Export CSV</Button>
                <AddCategoryDialog onCategoryChanged={fetchData} />
                <AddTransactionDialog accounts={accounts} categories={categories} supportsCategoryIds={supportsTransactionCategoryIds} onTransactionAdded={fetchData} />
              </>
            )}
            {initialTab === 'accounts' && <AddAccountDialog onAccountAdded={fetchData} />}
          </div>
        </div>
        ...
        <div>
          <Card className="border border-border/70 shadow-sm bg-card">
            ...
            <CardContent className="p-4">
              <form onSubmit={saveQuickExpense} className="grid grid-cols-1 gap-3 md:grid-cols-6">
                ...
                <div className="flex items-center md:col-span-1">
                  <Badge variant="secondary" className="w-full justify-center py-2">Today {formatCurrency(todaySpending)}</Badge>
                </div>
              </form>
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span>Week: <strong className="text-foreground">{formatCurrency(weekSpending)}</strong></span>
                <span className="text-border">•</span>
                <span>Daily guardrail: <strong className="text-foreground">{formatCurrency(suggestedDailyLimit || 0)}</strong></span>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            ...
            { label: 'Balance', value: formatCurrency(totalBalance), icon: Wallet, tone: 'text-blue-500' },
            ...
            <Card key={label} className="border-border/70 shadow-none">
              <CardContent className="flex items-center gap-3 p-3 sm:p-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className={`truncate text-base font-bold sm:text-lg ${tone}`}>{value}</p>
                  </div>
              </CardContent>
            </Card>
          ))}
              ...
              <CardHeader className="border-b bg-muted/20">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Search, filter, and review every movement of money.</p>
                  </div>
                  <div className="grid grid-cols-3 gap-2 sm:min-w-[430px]">
                    <div className="rounded-xl border bg-background px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Income</p><p className="truncate font-bold text-emerald-600">{formatCurrency(filteredIncome)}</p></div>
                    <div className="rounded-xl border bg-background px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Expense</p><p className="truncate font-bold text-rose-600">{formatCurrency(filteredExpenses)}</p></div>
                    <div className="rounded-xl border bg-background px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Net</p><p className={cn('truncate font-bold', filteredIncome - filteredExpenses >= 0 ? 'text-emerald-600' : 'text-rose-600')}>{formatCurrency(filteredIncome - filteredExpenses)}</p></div>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-5 p-4 sm:p-6">
                <div className="rounded-2xl border bg-muted/20 p-3">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_auto]">
                    <Input type="date" aria-label="From date" value={txFromDate} onChange={(e) => setTxFromDate(e.target.value)} />
                    <Input type="date" aria-label="To date" value={txToDate} onChange={(e) => setTxToDate(e.target.value)} />
                    <Button type="button" variant="ghost" onClick={clearTransactionFilters} disabled={!hasTransactionFilters}>Clear filters</Button>
                  </div>
                </div>

                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="text-sm font-medium">{filteredTransactions.length} transaction{filteredTransactions.length === 1 ? '' : 's'} found</p>
                  <Badge variant="outline">Showing {paginatedTransactions.length} on this page</Badge>
                </div>
            ...
            <Card className="overflow-hidden border-emerald-500/20">
              <CardHeader className="bg-emerald-500/5">
                ...
                <p className="text-sm text-muted-foreground">
                  Enter what you actually spend each day on breakfast, lunch, dinner, and snacks.
                </p>
              </CardHeader>
              <CardContent className="space-y-6 p-6">
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl bg-emerald-500/10 p-4">
                    <p className="text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-400">Food budget</p>
                    <p className="mt-1 text-2xl font-bold">{formatCurrency(foodBudget)}</p>
                  </div>
                  <div className="rounded-2xl bg-muted p-4">
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Entered expenses</p>
                    <p className="mt-1 text-2xl font-bold">{formatCurrency(totalFoodExpenses)}</p>
                  </div>
                  <div className="rounded-2xl bg-primary/10 p-4">
                    <p className="text-xs font-medium uppercase tracking-wide text-primary">Budget remaining</p>
                    <p className={`mt-1 text-2xl font-bold ${foodBudget - totalFoodExpenses < 0 ? 'text-destructive' : ''}`}>{formatCurrency(foodBudget - totalFoodExpenses)}</p>
                  </div>
                </div>

                {foodBudget === 0 && (
                  <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                    Add a Food budget for {format(mealPlanDate, 'MMMM yyyy')} in the Budgets tab and it will appear here automatically.
                  </div>
                )}

                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                  {(Object.keys(mealTotals) as Array<keyof MealEntry>).map((meal) => (
                    <div key={meal} className="rounded-xl border p-4">
                      ...
                      <p className="mt-1 text-xl font-bold">{formatCurrency(mealTotals[meal])}</p>
                    </div>
                  ))}
                </div>

                <div>
                  <h3 className="mb-3 font-semibold">Daily plan</h3>
                  <div className="max-h-[440px] overflow-auto rounded-xl border">
                    <div className="sticky top-0 z-10 grid min-w-[760px] grid-cols-6 gap-3 border-b bg-muted px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      <span>Day</span><span>Breakfast</span><span>Lunch</span><span>Dinner</span><span>Snacks</span>
                      <span>Daily total</span>
                    </div>
                    {mealPlanDays.map((date) => {
                      const entry = mealEntries[String(date.day)] || EMPTY_MEAL_ENTRY;
                      const dailyTotal = Object.values(entry).reduce((sum, amount) => sum + Number(amount || 0), 0);
                      return (
                        <div key={date.day} className="grid min-w-[760px] grid-cols-6 items-center gap-3 border-b px-4 py-2 text-sm last:border-0 hover:bg-muted/40">
                          <span className="font-medium">{date.label}</span>
                          {(Object.keys(entry) as Array<keyof MealEntry>).map((meal) => (
                            <Input key={meal} type="number" min="0" step="0.01" value={entry[meal] || ''} placeholder="0" onChange={(event) => updateMealExpense(date.day, meal, Number(event.target.value))} className="h-9" aria-label={`${date.label} ${meal} expense`} />
                          ))}
                          <span className="font-semibold">{formatCurrency(dailyTotal)}</span>
                        </div>
                      );
                    })}
                  </div>
                  <div className="mt-4 flex justify-end"><Button onClick={saveMealPlan}>Save Daily Expenses</Button></div>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
